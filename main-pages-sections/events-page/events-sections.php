<?php
    ...
    // Upcoming events — date >= today, soonest first
    ...
    // Past events — date < today, most recent first
    ...
    <div class="custom-container">
        ...
        <div class="flex justify-center gap-[0px] w-fit mx-auto mt-[40px] border-b-[2px] border-[#e0e0e0]">
            ...
            >
                ...
                <?php if ($past_events->found_posts > 0) : ?>
                    <span class="ml-[6px] text-[12px] bg-[#cccccc] text-[#555] rounded-full px-[8px] py-[2px]" id="past-count-badge">
                        <?php echo $past_events->found_posts; ?>
                    </span>
                <?php endif; ?>
            </button>
        </div>
        ...
        <div id="panel-upcoming">
            <?php
                ...
                include locate_template('main-pages-sections/events-page/upcoming-events.php');
            ?>
        </div>
...
<script>
    function switchTab(tab) {
        const upcoming = document.getElementById('panel-upcoming');
        const past     = document.getElementById('panel-past');
        const btnUp    = document.getElementById('tab-upcoming');
        const btnPast  = document.getElementById('tab-past');
        const badgePast = document.getElementById('past-count-badge');

        if (tab === 'upcoming') {
            upcoming.classList.remove('hidden');
            past.classList.add('hidden');

            btnUp.classList.add('text-[#1F773A]', 'border-[#1F773A]');
            btnUp.classList.remove('text-[#888888]', 'border-transparent');

            btnPast.classList.add('text-[#888888]', 'border-transparent');
            btnPast.classList.remove('text-[#1F773A]', 'border-[#1F773A]');
            if (badgePast) { badgePast.classList.replace('bg-[#1F773A]', 'bg-[#cccccc]'); badgePast.classList.replace('text-white', 'text-[#555]'); }

        } else {
            past.classList.remove('hidden');
            upcoming.classList.add('hidden');

            btnPast.classList.add('text-[#1F773A]', 'border-[#1F773A]');
            btnPast.classList.remove('text-[#888888]', 'border-transparent');

            btnUp.classList.add('text-[#888888]', 'border-transparent');
            btnUp.classList.remove('text-[#1F773A]', 'border-[#1F773A]');
            if (badgePast) { badgePast.classList.replace('bg-[#cccccc]', 'bg-[#1F773A]'); badgePast.classList.replace('text-[#555]', 'text-white'); }
        }
    }
</script>
